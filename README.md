...
## app-evoked documentation

1) Input file is:
    * `epoch/fif` epoch data file
    

2) Input parameters:
    * `ch_type` The channel type to plot. For 'grad', the gradiometers are collected in pairs and the RMS for each pair is plotted.
    If None the first available channel type from order shown above is used. Defaults to None.
    * `timepoints_from` The starting time point(s) to plot.
    * `timepoints_to` The final time point(s) to plot.
    * `step` Step of time point(s) to be plot.


3) Ouput files are:
    * `evoked/fif` evoked file.
